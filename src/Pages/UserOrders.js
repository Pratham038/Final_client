...
useEffect(() => {
    fetch(`https://frefishserver.onrender.com/api/orders/${user.email}`)
  ...
  return (
  <Wrapper>
  <h1>Thank-You for ordering {user.name}</h1>
  ...
  {orders.map(order => (
    ...
    <div className="orders">
      <ul>
        {order.cart.map(item => (
          <li key={item.id}>
            {item.amount} x {item.name} ({item.quantitys}) - <FormatPrice price={item.price} />
          </li>
        ))}
      </ul>
      </div>
      ...
      <p>Total price: <FormatPrice price={order.total_price + order.shipping_fee } /></p>
    </div>
    </div>
  ))}
</div>
